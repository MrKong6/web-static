...
class View extends React.Component {
    ...
    componentDidMount() {
        const request = async () => {
            try {
                let data = await ajax('/sales/through/query.do', {id: this.state.id});
                let list = await ajax('/sales/through/list.do', {id: this.state.id});
    ...
    delAction() {
        const request = async () => {
            try {
                await ajax('/sales/through/del.do', {id: this.state.id});
                this.setState({redirectToList: true});
            } catch (err) {
                if (err.errCode === 401) {
                    this.setState({redirectToReferrer: true})
                    ...
                    <nav aria-label="breadcrumb">
                        <ol className="breadcrumb location_bottom">
                            ...
                            <li className="breadcrumb-item"><Link
                                to={{
                                    pathname: `/home/sales/through/student/${this.state.id}`,
