...
import ajax, {ajaxJson} from "../../../utils/ajax";
...
class Editor extends React.Component {
    ...
    updated() {
        ...
        if(query.courseRelIds && query.courseRelIds.length > 0){
            query.courseId = changeArrayItemToString(query.courseRelIds);
            query.courseRelIds = null;
        }
        const request = async () => {
            try {
                await ajax('/academy/class/mod.do', query);
